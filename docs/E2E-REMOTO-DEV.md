...
**Nova execução em 24/09/2026:** `verify-remote-dev.ts` concluiu 11 PASS (TLS 1.3, capacidades runtime, contextos locais/restauração, rollback e HTTP health/ready/recusa de Bearer ausente ou desconhecido). Seu passo autenticado fica BLOCKED por procurar `HVB_E2E_API_TOKEN` no env; o token já existe no arquivo privado separado `e2e-fixture.json`, consumido pelo novo `verify-human-access-remote.ts`. Este comprovou o Bearer existente em `hvb.autenticar`, mas a jornada humana parou por falta de `acesso:administrar` no catálogo e na atribuição da fixture. Não solicitar novamente credencial/CA. Detalhes e pedido pontual ao banco em [LOGIN-HUMANO-077.md](LOGIN-HUMANO-077.md).
