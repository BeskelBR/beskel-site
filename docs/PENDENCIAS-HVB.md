...
| Ambiente de homologação e verificação MVP 10 | Sessão/proxy preparados apenas para HTTP loopback; hospedagem de API/PostgreSQL ainda não definida | Definir destino, TLS/Secure, operação/backup e sessões para ambiente escolhido; nenhuma infraestrutura externa autorizada por inferência |
