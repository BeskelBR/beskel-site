...
| Contrato de conexão e identidade entre API, frontend e banco | PENDENTE DE INTEGRAÇÃO: backend ainda limita conexão a banco local, exige papel hvb_app e contexto transacional hvb.org; login direto aceita credencial opaca de 64 caracteres hexadecimais, não JWT | Obter respostas dos chats de banco e frontend usando COORDENACAO-CHATS.md; documentar identidade, conexão/pool/TLS e destino da API antes de adaptar o backend. Nenhuma conexão remota ou mudança de autenticação foi realizada |
