...
| Gestão manual de funcionários e autorizações no frontend DEV/ADM | REQUISITO EXPLÍCITO DO USUÁRIO: operações rotineiras devem ser feitas pela interface, sem chat/SQL. Cadastro inicial integrado; manutenção posterior pela tela ainda pendente | Frontend implementar/validar consulta, edição de nome/login, concessão e revogação/restauração de atribuições usando contratos existentes; instruções em CONTRATOS-BACKEND-MVP11.md. Preservar autorização administrativa, histórico e controle de versão; distinguir manutenção de atribuições de edição dos componentes de um papel |
