...
| Ativação humana junto ao cadastro administrativo | Decisão recebida: CPF, temporária por email e redefinição obrigatória. 077 literal recuperada do Supabase e hash canônico confirmado; Argon2id/fronteira de envio preparados, endpoints ainda pendentes | BANCO — decisão/novo delta necessário: consumo de temporária retorna 42702 local/remoto; emissão de recuperação mantém sessão anterior em teste local. Não modificar 077 nem criar 078 por inferência. CA/serviço de email e integração BFF ainda pendentes. Evidência e pedido em LOGIN-HUMANO-077.md |
...
| Contrato de conexão e identidade entre API, frontend e banco | PARCIAL: backend implementado/testado localmente; responsável informou identidade de runtime provisionada e disponibilizou configuração privada. Primeiro E2E remoto tentou conexão e falhou na confiança TLS, antes de SQL | Corrigir cadeia confiável do cliente via HVB_DATABASE_CA_PEM; disponibilizar credencial HVB sintética para a etapa autenticada. Não pedir novo provisionamento de runtime já informado como concluído. Evidência e pedido ao banco em E2E-REMOTO-DEV.md; E2E remoto ainda não aprovado |
