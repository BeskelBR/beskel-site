...
| Ativação humana junto ao cadastro administrativo | 077 canônica preservada; Argon2id/fronteira de envio preparados, endpoints pendentes. Verificador atualizado sem ativação direta: consumo real, recusa de reuso e três provas de revogação na recuperação; nova execução ainda não realizada | BANCO — concluir correção autorizada do 42702 e da revogação na recuperação, sem reescrever 077 ou inferir 078. Depois: 14 PASS, regressões e handlers HTTP. CA resolvida; serviço de email, integração BFF e ambiente de teste fora de OneDrive pendentes. Evidência e pedido em LOGIN-HUMANO-077.md |
...
| Contrato de conexão e identidade entre API, frontend e banco | TLS RESOLVIDO em 24/09: Node/pg + identidade runtime completaram handshake real TLS 1.3 com certificado/hostname validados; CA configurada apenas no ambiente privado | E2E autenticado ainda não aprovado: confirmar fixture HVB sintética e executar etapas restantes após correção canônica. Não pedir novo provisionamento de runtime/CA. Histórico preservado em E2E-REMOTO-DEV.md; prova de transporte não equivale à jornada humana |
...
| PostgreSQL sob OneDrive | TESTES LOCAIS COM ESCRITA SUSPENSOS após alerta de exclusão relatado; origem exata dos arquivos não confirmada. Verificador 077 recusa data_directory sob OneDrive antes de BEGIN | Responsável pelo banco deve preparar teste fora da sincronização, com configuração privada. Nenhum arquivo/cluster será apagado ou movido por inferência; revisar localização e backup consistente |
