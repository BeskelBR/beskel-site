...
| Contrato de conexão e identidade entre API, frontend e banco | PARCIAL: delta de 23/09 implementa opt-in remoto DEV com TLS verificado, readiness por herança/capacidades e search_path/hvb.org locais à transação; 30 testes locais passaram. Bearer opaco preservado | Falta identidade PostgreSQL LOGIN herdeira de hvb_app e configuração privada de runtime para E2E Supabase; conexão real remota ainda não executada. Histórico, configuração e evidências em DELTA-BACKEND-POSTGRES-DEV.md; aguardar retorno do frontend sem alterar sua estrutura |
