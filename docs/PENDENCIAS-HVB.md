...
| Ativação humana junto ao cadastro administrativo | Hotfix observado no Supabase reproduzido no novo teste isolado: 14 PASS / 0 FAIL / 0 BLOCKED, incluindo consumo/reuso e revogação; 14 regressões API/NFC PASS. 077 canônica intacta, nenhuma 078 | BANCO — preparar acesso:administrar no catálogo e a atribuição global do administrador sintético da fixture remota; ausência bloqueou a emissão pelo runtime. Depois repetir jornada remota e implementar handlers HTTP, integração BFF/email. Argon2id/fronteira preparados; histórico dos defeitos resolvidos e pedido atual em LOGIN-HUMANO-077.md |
