...
| Cadastro integrado com NFC | Backend implementa vínculo atômico opcional no onboarding e leitura paginada de cartões por funcionário; serviço canônico C18 reaproveitado sem modificação | Frontend integrar captura/gestão; leitor físico e jornada remota não testados. Mesma tag não pode ser reciclada ou vinculada a várias unidades pelo contrato congelado. NFC não é senha nem prova biométrica |
| Ativação humana junto ao cadastro administrativo | Delta autorizado: proposta de login/senha com ativação pelo próprio funcionário, sujeita à preferência consultada; não há endpoint humano implementado | Banco devolver contrato de persistência de identidade/ativação/tentativas/revogação; backend implementar após alinhamento e frontend integrar BFF. Não usar credencial API como senha nem habilitar Supabase Auth por inferência. Pedido copiável em CADASTRO-LOGIN-NFC.md |
