...
Atualização de 25/09/2026 — [estoque backend 111](VERIFICACAO-ESTOQUE-111.md): jornada episódio → catálogo/saldo → entrada → consumo conciliado → estorno físico **VERIFICADA**, 73 PASS / 0 FAIL / 0 BLOCKED por HTTP real + Node/pg Supabase. Saldo 0 → 20 → 17 → 20; recusas de excesso, idempotência, RLS e 403 sem permissão comprovados. Nenhuma lacuna de API ou alteração de banco/domínio encontrada. Baixa restrita a essa jornada com rollback; worker/outbox, frontend, login humano e homologação com commits independentes continuam pendentes. Evidência temporal da primeira tentativa classificada C (fixture), corrigida no roteiro, preservada no relatório.

Atualização de 25/09/2026 — [desbloqueio Supabase 111](SINCRONIZACAO-SUPABASE-111.md): **resolvidos por evidência** EXECUTE de cpf_valido e RBAC da fixture remota A. Jornada responsável → paciente/vínculo → episódio → ocupação: 38 PASS / 0 FAIL / 0 BLOCKED remoto; regressões locais 31/31. Migrations 103–111 literais, 111 hashes conferidos e ready atualizado. A fixture nominal A não existe no TEST: reprodução local da mesma identidade permanece configuração pendente, sem criação de usuários/papéis. Nova pendência **BACKEND — worker/runner outbox**: adaptar chamadas e grants legados às funções/escopo das migrations 109–110 antes de rodar worker ou reprovisionar pelo runner geral; não pedir relaxamento ao banco. Login humano, frontend, quatro erros de tipo do gateway e limitações de homologação permanecem. O registro 102 abaixo é histórico; suas falhas de CPF/RBAC foram superadas somente no escopo demonstrado pela 111.

Atualização de 25/09/2026 — [sincronização Supabase 102](SINCRONIZACAO-SUPABASE-102.md): migrations 078–102 recuperadas literalmente, 102 hashes conferidos; contratos cadastrais, buscas, views e readiness sincronizados. Testes locais: 28 PASS / 3 FAIL (EXECUTE de cpf_valido ausente no runtime). Remoto: TLS/hashes/ready/Bearer aprovados, jornada bloqueada por essa capacidade e permissões da fixture. Frontend precisa incorporar vínculo inicial obrigatório no POST paciente, campos/leituras novos e autocomplete de CEP; quatro erros de tipo no gateway recebido continuam a cargo desse responsável. Nenhum desses bloqueios foi reclassificado como homologação. Login humano local em andamento foi preservado fora desta publicação; seu antigo bloqueio de acesso:administrar na fixture foi resolvido e o diagnóstico de funções humanas remoto passou 20/0.
...
| Gestão manual de funcionários e autorizações no frontend DEV/ADM | PARCIAL: cadastro e manutenção administrativa entregues pelo frontend até 354ac88 e incorporados; falta de código da manutenção superada, sem declarar homologação | Validar jornada integrada; integrar NFC e futura ativação humana conforme CADASTRO-LOGIN-NFC.md. Preservar autorização administrativa, histórico e controle de versão; distinguir manutenção de atribuições de edição dos componentes de um papel |
| Cadastro integrado com NFC | Backend implementa vínculo atômico opcional no onboarding e leitura paginada; C18 preservado. Frontend integrou cadastro/consulta/vínculo/revogação até 3c0abe1, com evidência isolada do responsável | Falta de integração no código frontend superada; leitor físico e jornada remota continuam não testados. Mesma tag não pode ser reciclada ou vinculada a várias unidades pelo contrato congelado. NFC não é senha nem prova biométrica |
| Ativação humana junto ao cadastro administrativo | Histórico: hotfix reproduzido no TEST isolado com 14 PASS / 0 FAIL / 0 BLOCKED e 14 regressões API/NFC PASS. Atualização 25/09: catálogo/atribuição acesso:administrar resolvidos; diagnóstico de funções humanas remoto passou 20/0. A 078 canônica foi incorporada literalmente nesta sincronização, preservando a 077 | Concluir e testar handlers HTTP ainda locais, integrar BFF/email em delta próprio. Não confundir essa prova de funções com aprovação HTTP. Histórico em LOGIN-HUMANO-077.md; sincronização e bloqueios cadastrais distintos em SINCRONIZACAO-SUPABASE-102.md |
| Contratos de backend solicitados pelo frontend MVP 11 | RESOLVIDA a falta de leitura de permissões por papel, onboarding atômico, entrada atômica de lote novo e busca server-side de pacientes; 46 testes locais aprovados. Frontend integrou contratos até 56cd405, com evidência própria de verificações isoladas | Resta jornada integrada API/banco/interface; catálogo permanece global na organização e login humano não implementado. Limites em CONTRATOS-BACKEND-MVP11.md; evidência atribuída ao frontend em evidencias/frontend-mvp11-integracao.json |
| Seleção de local na interface de entrada de estoque | DEPENDÊNCIA DE PERMISSÃO informada pelo frontend: GET /v1/locais exige locais:ler na unidade, além das permissões do comando de entrada | Definir papéis operacionais com locais:ler ou solicitar contrato específico de consulta. UI já restringe unidades elegíveis; não ampliar RBAC por inferência. Não bloqueia o handshake TLS |
| Login humano, emissão/renovação/recuperação de credenciais, MFA/step-up e limites de abuso | PARCIAL: MVP 10 entrega sessão web local com cookie, timeout, logout, limite local de tentativas e contexto próprio; entrada ainda usa credencial DEV | Implementar login operacional/recuperação/MFA e controles de ambiente antes de exposição externa; histórico da parte resolvida em ADR 0029 |
| Ambiente de homologação e verificação MVP 10 | Sessão/proxy preparados apenas para HTTP loopback; banco/Supabase atribuído pelo usuário a outro chat em 23/09; hospedagem da API e compatibilidade ainda não confirmadas | Responsáveis separados em COORDENACAO-CHATS.md; frontend trata sessão/TLS, banco trata configuração/operação/backup, este chat trata adaptação da API após alinhar os contratos; nenhuma infraestrutura externa autorizada por inferência |
| Contrato de conexão e identidade entre API, frontend e banco | TLS, capacidades runtime, contexto/rollback e HTTP health/ready remotos PASS; Bearer sintético aceito por hvb.autenticar via Node/pg. CA privada, certificado/hostname validados | Jornada humana remota ainda bloqueada pelo catálogo/permissão administrativa da fixture; não pedir nova identidade runtime, CA ou token já disponibilizados. Histórico em E2E-REMOTO-DEV.md e LOGIN-HUMANO-077.md |
| Verificação funcional pelos chats responsáveis | Usuário esclareceu em 23/09 que os chats já estão autorizados a executar testes nos respectivos blocos; este chat evita duplicação e mantém integridade por desenvolvimento | Receber evidências por ambiente/escopo/SHA; restauração/logout/troca de abas e alterações finais do MVP 10 permanecem pendentes até comprovação, não por falta de autorização |
| Lint histórico do frontend MVP 8 | 21 diagnósticos em system-v2 até system-v8, idênticos à baseline publicada antes do MVP 10; nenhum diagnóstico novo nesses arquivos | Tratar limpeza de callbacks/variáveis/diretivas em lote próprio, sem confundir com falha nova de integridade |
...
| PostgreSQL sob OneDrive | Novo teste criado fora da sincronização por autorização explícita em 24/09; 001–077 e hotfix observado provisionados, 28 verificações locais PASS. Cluster antigo preservado, sem testes novos nele | Origem exata do alerta anterior não confirmada; localização/operação do DEV antigo e backup consistente continuam pendentes. Usar configuração privada do novo TEST; não executar scripts antigos que reiniciem testes no cluster sincronizado |
