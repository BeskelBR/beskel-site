CREATE OR REPLACE FUNCTION hvb.reservar_outbox(limite integer, token uuid) RETURNS SETOF hvb.outbox
...
BEGIN
 ...
 AND tentativas>=5 AND lease_ate<now() ORDER BY lease_ate,id LIMIT limite FOR UPDATE SKIP LOCKED
 ) UPDATE outbox o SET pendente_em=now(),ultimo_erro='tentativas_esgotadas'
 FROM esgotados e WHERE o.id=e.id;
