...
DECLARE m modelo_documento_versao;c campo_modelo_documento;k text;v jsonb;resto text;saida text;inicio integer;fim integer;marcador text;
BEGIN
 ...
 marcador:=left(resto,fim-1);
 IF marcador!~'^[a-z][a-z0-9_]{0,39}$' OR NOT EXISTS(SELECT 1 FROM campo_modelo_documento WHERE organizacao_id=p_org AND modelo_versao_id=p_modelo AND campo_modelo_documento.codigo=marcador) THEN RAISE EXCEPTION 'Marcador desconhecido' USING ERRCODE='23514';END IF;
 saida:=saida||coalesce(p_campos->>marcador,'');resto:=substr(resto,fim+2);
