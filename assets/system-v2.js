(() => {
  ...
  const modules = {
    ...
    internacao: {
      ...
      description: "Acompanhamento de episódios, ocupações, classificações e regras configuráveis de diária.",
    ...
    estoque: {
      ...
      description: "Posições, lotes, reservas, movimentações e inventário conectados à operação física.",
    ...
    financeiro: {
      ...
      description: "Títulos, recebimentos, caixa, créditos e conciliação financeira.",
      next: "Organizar indicadores e rotinas de exceção depois da validação operacional com a gestão.",
      views: [
    ...
    compras: {
      ...
      description: "Pedidos, recebimentos, preços negociados e custo analítico de aquisição.",
    ...
    documentos: {
      ...
      description: "Solicitações, versões, aprovações, assinaturas e entregas documentais registradas.",
    ...
    preventivo: {
      ...
      description: "Adesões, ocorrências, aplicações e revisões de protocolos preventivos.",
    ...
    administracao: {
      ...
      description: "Usuários, papéis, unidades, dispositivos e credenciais do ecossistema HVB.",
  ...
  const refreshModule = $("#refresh-module");
  const loadMoreButton = $("#load-more");

  const isLocal = ["localhost", "127.0.0.1"].includes(location.hostname);
  const localDefault = isLocal ? location.origin : location.origin;
  let apiBase = localStorage.getItem("hvb-api-base") || localDefault;
  let token = sessionStorage.getItem("hvb-access-token") || "";
  let actor = null;
  let organization = null;
  let activeUnit = localStorage.getItem("hvb-unit-id") || null;
  ...
  function setInfrastructure(status, text) {
    apiStatus.textContent = text;
    apiStatus.className = `api-status ${status}`.trim();
  }
  ...
  async function checkInfrastructure() {
    try {
      ...
      setInfrastructure(ok ? "ok" : "error", ok ? "API online" : "API instável");
      healthCard.textContent = ok ? "Online" : "Instável";
    } catch {
      setInfrastructure("error", "API indisponível");
  ...
  async function loadContext() {
    ...
    if (!activeUnit) {
      try {
        ...
        if (activeUnit) localStorage.setItem("hvb-unit-id", activeUnit);
      } catch {
        activeUnit = null;
      }
    }
  ...
  loginForm.addEventListener("submit", async (event) => {
    ...
    activeUnit = unitInput.value.trim() || null;
    localStorage.setItem("hvb-api-base", apiBase);
    if (activeUnit) localStorage.setItem("hvb-unit-id", activeUnit);
    else localStorage.removeItem("hvb-unit-id");

    if (!/^[a-f0-9]{64}$/.test(candidate)) {
      setFeedback("A credencial DEV deve conter exatamente 64 caracteres hexadecimais.", "error");
      return;
    }
    if (activeUnit && !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(activeUnit)) {
      setFeedback("O identificador da unidade deve ser um UUID válido.", "error");
      return;
    }
    ...
    try {
      ...
      else setFeedback("Não foi possível conectar ao backend configurado. Verifique a API e o ambiente DEV.", "error");
    } finally {
  ...
  function renderModuleGrid() {
    ...
    Object.entries(modules).forEach(([key, item]) => {
      ...
      state.textContent = `${item.views.length} visões de consulta`;
  ...
  function navigate(module) {
    ...
    const dashboard = $('[data-module-view="dashboard"]');
    const detail = $('[data-module-view="detail"]');
  ...
  function renderTabs(module) {
    ...
    module.views.forEach((view) => {
      ...
      button.addEventListener("click", () => selectView(view.key));
  ...
  function selectView(key) {
    ...
    activeView = module.views.find((view) => view.key === key) || module.views[0];
    moduleTabs.querySelectorAll(".module-tab").forEach((button) => {
      const active = button.dataset.view === activeView.key;
  ...
  refreshModule.addEventListener("click", () => {
    if (activeModule === "dashboard" || !activeView) return;
    currentItems = [];
    nextCursor = null;
    loadView(false);
  });
  ...
  loadMoreButton.addEventListener("click", () => {
    if (!loading && nextCursor) loadView(true);
  });

  moduleSearch.addEventListener("input", () => renderCurrentItems());

  function buildPath(view, cursor = null) {
    const url = new URL(view.endpoint, "https://local.invalid");
    if (view.unit) {
      if (!activeUnit) return null;
      url.searchParams.set("unidade_id", activeUnit);
    }
  ...
  async function loadView(append) {
    if (!activeView || loading) return;
    const path = buildPath(activeView, append ? nextCursor : null);
    if (!path) {
      renderEmpty("Unidade não definida", "Informe uma unidade hospitalar válida na configuração DEV para consultar esta visão.");
      moduleCount.textContent = "0 registros";
      moduleContext.textContent = "Unidade necessária";
      loadMoreButton.hidden = true;
      return;
    }

    loading = true;
    moduleContext.textContent = `${activeView.label} • consultando`;
    if (!append) {
      moduleContent.replaceChildren();
      const indicator = document.createElement("div");
      indicator.className = "loading";
      indicator.textContent = "Carregando dados disponíveis";
      moduleContent.append(indicator);
    }
    ...
    try {
      ...
      moduleContext.textContent = `${activeView.label} • consulta autenticada`;
      loadMoreButton.hidden = !nextCursor;
      renderCurrentItems();
    } catch (error) {
      loadMoreButton.hidden = true;
      if (error.status === 403) {
        renderEmpty("Acesso não liberado", "O perfil autenticado não possui permissão para consultar esta visão.", true);
        moduleContext.textContent = "Permissão insuficiente";
      } else if (error.status === 401) {
        renderEmpty("Sessão inválida", "A credencial expirou ou foi revogada. Entre novamente no sistema.", true);
        moduleContext.textContent = "Sessão inválida";
      } else {
        renderEmpty("Consulta indisponível", "A visão está prevista no frontend, mas o backend DEV não respondeu conforme o contrato esperado.");
        moduleContext.textContent = "Falha de integração DEV";
      }
      moduleCount.textContent = "0 registros";
    } finally {
      loading = false;
  ...
  function filteredItems() {
    const term = moduleSearch.value.trim().toLocaleLowerCase("pt-BR");
    if (!term) return currentItems;
    return currentItems.filter((item) =>
      Object.values(item).some((value) => {
        if (value === null || value === undefined) return false;
        if (typeof value === "object") return JSON.stringify(value).toLocaleLowerCase("pt-BR").includes(term);
  ...
  function renderCurrentItems() {
    ...
    moduleCount.textContent = `${items.length} ${items.length === 1 ? "registro" : "registros"}${moduleSearch.value.trim() ? ` filtrados de ${currentItems.length}` : ""}`;
    if (!items.length) {
      renderEmpty(
        currentItems.length ? "Nenhum resultado no filtro" : "Nenhum registro disponível",
        currentItems.length ? "Ajuste o termo de busca para ampliar os resultados." : "A consulta foi concluída, mas não retornou itens para esta sessão.",
      );
      return;
    }
    renderSummary(items);
  ...
  function renderSummary(items) {
    ...
    const total = summaryCard("Registros visíveis", String(items.length));
    const fields = summaryCard("Campos exibidos", String(Math.min(Object.keys(items[0]).length, 7)));
    const page = summaryCard("Paginação", nextCursor ? "Há mais dados" : "Página final");
    summary.append(total, fields, page);
    moduleContent.append(summary);
  }
  ...
  function renderEmpty(titleText, detailText, permission = false) {
    moduleContent.replaceChildren();
    const box = document.createElement("div");
    box.className = "empty-state";
    if (permission) box.classList.add("permission-note");
  ...
  function humanize(key) {
    const labels = {
      ...
      criado_em: "Criado em",
      criada_em: "Criada em",
      admitido_em: "Admissão",
      encerrado_em: "Encerramento",
      recebido_em: "Recebido em",
      ocorrida_em: "Ocorrida em",
      produzido_em: "Produzido em",
    ...
    return key.replace(/_id$/, "").replaceAll("_", " ").replace(/\b\w/g, (char) => char.toUpperCase());
  }

  function statusClass(text) {
    const value = String(text).toLocaleLowerCase("pt-BR");
    if (["ativo", "aberta", "aberto", "confirmado", "liberado", "vigente", "concluido", "concluído", "pago"].some((token) => value.includes(token))) return "good";
    if (["pendente", "rascunho", "revisao", "revisão", "parcial", "programado"].some((token) => value.includes(token))) return "warn";
    if (["cancel", "revog", "encerrado", "expirado", "revertido", "inativo", "falha"].some((token) => value.includes(token))) return "bad";
    return "";
  }
  ...
  function renderTable(items) {
    const preferred = Object.keys(items[0]).filter((key) => key !== "organizacao_id" && key !== "comando_id" && !key.startsWith("hash_"));
    const priority = ["nome", "descricao", "tipo", "situacao", "estado", "referencia", "valor", "saldo", "criada_em", "criado_em", "id"];
    const columns = [...priority.filter((key) => preferred.includes(key)), ...preferred.filter((key) => !priority.includes(key))].slice(0, 7);
    ...
    items.forEach((item) => {
      ...
      columns.forEach((key) => {
        ...
        const formatted = formatValue(item[key], key);
        if (key === "id" || key.endsWith("_id")) {
          cell.classList.add("cell-id");
          cell.title = String(item[key] ?? "");
          cell.textContent = formatted;
        } else if (["situacao", "estado", "resultado"].includes(key) && formatted !== "—") {
          const pill = document.createElement("span");
          pill.className = `status-pill ${statusClass(formatted)}`.trim();
